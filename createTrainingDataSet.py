...
# origins = ["London Underground Zone 5-6 (Any)"]
# destinations = ["Shirley"]
# templates = ["I'd like to secure a train journey from {origin} to {destination} for {passengers} passengers on {date}."]
...
for dataset_num in range(1, 11):
  ...
  for i in range(100):
      ...
      template_entities_end_sequence = [{"label": "Origin", "value": template_origin_end}, {"label": "Destination", "value": template_destination_end}, {"label": "Passenger", "value": template_passengers_end}, {"label": "Date", "value": template_date_end}]

      template_entities_end_sequence_sorted = sorted(template_entities_end_sequence, key=lambda k: k['value']) 

      sentence = template.format(origin=origin, destination=destination, passengers=passengers, passengers_label=passengers_label, date=date)
      ...
      entities = {"entities": [(origin_start, origin_end, "Origin"),
      ...
      print(f"Dataset {dataset_num} : {i} created.")

  # Create dataset file
  with open(f"training_data/data_1m_{dataset_num}.py", "w") as file:
      file.write("TRAIN_DATA = " + str(data))
